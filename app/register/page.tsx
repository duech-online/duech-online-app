    ...
    return (
        <main className="flex items-center justify-center md:h-screen">
            <div className="relative mx-auto w-full max-w-[420px] space-y-3 p-4 md:-mt-32">
                ...
                <form action={formAction} className="space-y-3 rounded-lg bg-gray-50 px-6 pb-6 pt-6">
                    <label className="block">
                        <span className="mb-1 block text-xs font-medium text-gray-900">Nombre</span>
                        <input name="name" type="text" required className="w-full rounded-md border px-3 py-2" />
                    </label>
                    <label className="block">
                    ...
                    <label className="block">
                        ...
                        <input name="password" type="password" minLength={6} required className="w-full rounded-md border px-3 py-2" />
                    </label>
                    <input type="hidden" name="redirectTo" value={callbackUrl} />
                    <Button className="mt-2 w-full" aria-disabled={isPending}>Registrarme</Button>
                    {errorMessage && <p className="text-sm text-red-600">{errorMessage}</p>}
                    <p className="text-center text-sm text-gray-600">
                        ¿Ya tienes cuenta? <a className="text-blue-600 hover:text-blue-800" href={`/login?callbackUrl=${encodeURIComponent(callbackUrl)}`}>Inicia sesión</a>
                    </p>
                </form>
            </div>
        </main>
